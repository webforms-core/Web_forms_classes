...
defmodule WebFormsCore.WebForms do
  ...
  def then_(form, %__MODULE__{} = new_form) do
    data = new_form.data
    new_form =
      if data in [nil, ""] do
        new_form
  ...
  def then_(form, configure) when is_function(configure, 1) do
  ...
  def repeat(form, new_form, repeat_count) do
    if new_form == nil do
      form
    else
  ...
  def repeat(form, new_form, repeat_count, index) do
    if new_form == nil do
      form
    else
